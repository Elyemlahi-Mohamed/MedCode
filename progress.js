...
let tailwindCssProgress = document.querySelector(".tailwindCssProgress");
...
let tailwindCssCount = document.querySelector(".tailwindCssCount");
...
let tailwindCssStartValue = 0, tailwindCssEndValue = 85;
...
let tailwindCssInterval = setInterval(() => {

    tailwindCssStartValue++;
    tailwindCssProgress.style.background = `conic-gradient(#0087be ${tailwindCssStartValue * 3.6}deg, #ffffff 0deg)`;
    tailwindCssCount.innerHTML = `${tailwindCssStartValue}%`;

    if (tailwindCssStartValue == tailwindCssEndValue) {
        clearInterval(tailwindCssInterval);
    }
